...
async function ask_file_input (data: StringTableDataStorage, token?: vscode.CancellationToken, in_file?: vscode.Uri, current_document?: vscode.TextDocument): Promise<StringtableFileData | undefined> {
    if (in_file) { return await data.get_data_item_for_file(in_file); }
    ...
    selection_items.push(...data.all_stringtable_file_data_items.sort((a, b) => a.name.localeCompare(b.name)).map((item) => {
        return {
            label: item.name,
            iconPath: item.icon,
            kind: vscode.QuickPickItemKind.Default,
    ...
    const stringtable_data_item = await vscode.window.showQuickPick(selection_items,
        {
    ...
    if ((!stringtable_data_item) || (token?.isCancellationRequested)) { return; }
...
async function ask_container_name_input (existing_container_names: string[], token?: vscode.CancellationToken, container_name?: string): Promise<string | undefined> {
    ...
    container_name = await vscode.window.showQuickPick(["New container name"].concat(existing_container_names),
        {
...
const KEY_NAME_MODIFICATION_FUNCTIONS: ((key: string) => string)[] = [
    (key) => { return key.trim(); },
    (key) => { return key.replace(/^str_/gmi, "STR_"); },
];
...
async function ask_original_value_input (token?: vscode.CancellationToken, original_value?: string): Promise<string | undefined> {
    ...
    if (original_value) { return original_value; }
    if (token?.isCancellationRequested) { return; }

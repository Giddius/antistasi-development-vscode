...
export const ALL_SUB_EXTENSIONS: ReadonlyArray<SubExtension> = _handle_all_subextensions([stringtable_data]);
...
export function activate_all_sub_extensions (context: vscode.ExtensionContext) {
    return Promise.allSettled(
        ALL_ENABLED_SUB_EXTENSIONS.map((sub_extension) => {
            return sub_extension.activate_sub_extension(context);
        })
    );
}
...
export function deactivate_all_sub_extensions () {
    return Promise.allSettled(
        ALL_ENABLED_SUB_EXTENSIONS.map((sub_extension) => {
            return sub_extension.deactivate_sub_extension!();
        })
    );
}

...
function output_format (descriptive_name: string, target_file_extension: string, importance: number = 100) {
    const nomalized_target_file_extension = target_file_extension.trim().replace(/^\./gm, "").toLowerCase();

    return function actualDecorator (originalMethod: any, context: ClassMethodDecoratorContext<FoundKeysWriter, format_function>) {
        const spec = { descriptive_name: descriptive_name, target_file_extension: nomalized_target_file_extension, importance: importance, format: originalMethod };

        context.addInitializer(function () {
            const klass = Object.getPrototypeOf(this);
            klass.constructor.available_output_formats.push(spec);
            if (this["output_format_map"] === undefined) {
                this["output_format_map"] = new Map();
            }
            this["output_format_map"].set(spec.target_file_extension, spec.format.bind(this));
        });
    };
}
class FoundKeysWriter {
    ...
    public get_filters () {
        ...
        for (const output_spec of Array.from(this.get_output_formats()).sort((a, b) => {
    ...
    get_output_format (target_path: string): format_function | undefined {
        try {
...
async function _create_html (webview: vscode.Webview, undefined_keys: FoundKey[]): Promise<string> {
    ...
    _body += `\n<div class="misc-buttons-box">
                    ...
                    <button class="copy-all-key-names-for-discord-button">copy for Discord</button>
    ...
    for (const key_name of found_key_map.keys()) {
        ...
        for (const _key of found_key_map.get(key_name)!) {
            _body += `
            <li>
                file: <code class="path-code">${await _get_pretty_path(_key.file)}</code>, line number: <code>${_key.start_line + 1}</code>, character: <code>${_key.start_char}</code>   ${await _create_button(_key)}
            </li>\n`;
        }
...
async function create_discord_text (in_undefinded_keys: FoundKey[]): Promise<string> {
    const sqf_language_available: boolean = ((await vscode.languages.getLanguages()).map((item) => { return item.toLowerCase(); }).includes("sqf"));

    const found_key_map = await _to_undefined_key_map(in_undefinded_keys);

    const text = new vscode.MarkdownString("", true);

    for (const key_name of found_key_map.keys()) {
        text.appendMarkdown(`## ${key_name}\n`);

        for (const _key of found_key_map.get(key_name)!) {
            text.appendMarkdown(`- \`${_key.relative_path}\`\n`);
            text.appendMarkdown(`    - line number: \`${_key.range.start.line + 1}\`, character: \`${_key.range.start.character + 1}\``);
            if (_key.full_line) {
                text.appendCodeblock(_key.full_line, (sqf_language_available) ? "sqf" : "javascript");
            } else {
                text.appendMarkdown(`\n`);
            }
...
export async function create_undefined_stringtable_keys_result_web_view (undefined_keys: FoundKey[]) {
    ...
    const view = vscode.window.createWebviewPanel("undefinedStringtableKeys", "Undefined Stringtable Keys", column || vscode.ViewColumn.One, {
        enableScripts: true,
        localResourceRoots: resource_roots,
    ...
    const message_listener = view.webview.onDidReceiveMessage(async (message) => {
        switch (message.command) {
            ...
            case "copy_all_names":
                const all_names: string[] = Array.from(
                    new Set(
                        sorted_undefined_keys.map((item) => {
                            return item.text;
                        })
                    )
                );
            ...
            case "copy_all_names_for_discord":
                await vscode.env.clipboard.writeText(await create_discord_text(undefined_keys)).then(() => {
                    vscode.window.showInformationMessage("all undefined Stringtable keys copied to clipboard in Discords markdown format!");
                });
                break;
            ...
            case "save_to_file":
                ...
                await writer.write(target.fsPath);
